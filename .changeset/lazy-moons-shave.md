...
Cutoff glide timing is fixed: `glideTime` is a ramp duration, but it was passed straight to `setTargetAtTime` as an exponential time constant, so a glide took roughly 3x as long as requested. It is now divided by 3. The direct cutoff setters also honour `options.cancelPrevious` instead of always clearing scheduled automation.
