...
describe("Analyze distortion in init_sample.webm", () => {
  ...
  it("should analyze distortion through the full pipeline", async () => {
    ...
    console.log("\n=== CHANGES ===");
    console.log(
      `  Peak change: ${((processedStats.peak / originalStats.peak - 1) * 100).toFixed(1)}%`,
    );
    console.log(
      `  RMS change: ${((processedStats.rms / originalStats.rms - 1) * 100).toFixed(1)}%`,
    );
    console.log(
      `  Crest factor change: ${(processedStats.crestFactor - originalStats.crestFactor).toFixed(2)}`,
    );
    console.log(`  Sudden jumps change: ${processedStats.suddenJumps - originalStats.suddenJumps}`);
    console.log(
      `  Max jump change: ${(processedStats.maxJump - originalStats.maxJump).toFixed(4)}`,
    );
