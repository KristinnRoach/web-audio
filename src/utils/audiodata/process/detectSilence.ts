export function detectThresholdCrossing(
  ...
  const samples = Array.from({ length: numChannels }, (_, i) => audioBuffer.getChannelData(i));
  ...
  function findFromStart(): number {
    for (let i = 0; i < totalSamples; i++) {
      const maxAmplitude = Math.max(...samples.map((channel) => Math.abs(channel[i])));
  ...
  function findFromEnd(): number {
    ...
    return unit === "seconds" ? (totalSamples - 1) / sampleRate : totalSamples - 1;
  }
