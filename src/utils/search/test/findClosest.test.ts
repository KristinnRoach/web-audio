...
describe("findClosest", () => {
  ...
  const findClosestNaive = <T>(array: T[], target: number, getValue: (item: T) => number): T => {
    return array.reduce((prev, curr) =>
      Math.abs(getValue(curr) - target) < Math.abs(getValue(prev) - target) ? curr : prev,
    );
  };
  ...
  describe("Edge Cases", () => {
    it("throws error for empty array", () => {
      expect(() => findClosest([], 5, "any", (x) => x)).toThrow("Array cannot be empty");
    });
  ...
  describe("Custom Distance Function", () => {
    it("uses custom distance function for logarithmic scale", () => {
      ...
      const logDistance = (a: number, b: number) => Math.abs(Math.log2(a) - Math.log2(b));

      // Target is 330Hz - closer to 220 in log scale than 440
      const result = findClosest(frequencies, 330, "any", (x) => x, logDistance);
    ...
    it.skip("demonstrates O(log n) vs O(n) complexity", () => {
      ...
      sizes.forEach((size) => {
        ...
        for (let run = 0; run < 5; run++) {
          const targets = Array.from({ length: iterations }, () => Math.random() * size);
          ...
          totalNaiveTime += (performance.now() - naiveStart) * (iterations / naiveIterations);
        }
      ...
      console.log(`Binary ratios: ${binaryRatio1.toFixed(2)}, ${binaryRatio2.toFixed(2)}`);
      console.log(`Naive ratios: ${naiveRatio1.toFixed(2)}, ${naiveRatio2.toFixed(2)}`);
    });
  });
  ...
  describe("Real-world Use Cases", () => {
    it("finds closest musical note to frequency", () => {
      ...
      const exactResult = findClosest(notes, 440, "any", (note) => note.frequency);
      expect(exactResult.note).toBe("A4");

      // Test finding closest to 435Hz (should be A4)
      const closeResult = findClosest(notes, 435, "any", (note) => note.frequency);
    ...
    it("handles time-based searches efficiently", () => {
      ...
      const result = findClosest(timestamps, targetTime, "any", (sample) => sample.timeSeconds);
  ...
  describe("Robustness Tests", () => {
    ...
    it("works consistently with different data types", () => {
      ...
      // Test with dates
      const dates = [new Date("2023-01-01"), new Date("2023-06-01"), new Date("2023-12-01")];
      const targetDate = new Date("2023-08-01").getTime();
      const dateResult = findClosest(dates, targetDate, "any", (d) => d.getTime());
  ...
  describe("Stress Tests", () => {
    ...
    it("maintains accuracy with floating point arithmetic", () => {
      const preciseArray = Array.from({ length: 1000 }, (_, i) => (i * Math.PI) / 1000);
  ...
  describe("Default Parameter Convenience", () => {
    ...
    it("still works with explicit getValue for object arrays", () => {
      const objects = [{ value: 1 }, { value: 5 }, { value: 10 }, { value: 15 }, { value: 20 }];
