...
describe("Debug >1.0 peak issue", () => {
  ...
  it("should track where peaks exceed 1.0", async () => {
    ...
    const { detectThresholdCrossing } =
      await import("../../../utils/audiodata/process/detectSilence");
    const { trimAudioBuffer } = await import("../../../utils/audiodata/process/trimBuffer");

    // Step 1: Normalize
    const normalized = normalizeAudioBuffer(audioContext, buffer, 0.98);
    const peakAfterNorm = analyzeBuffer(normalized, "2. After normalize to 0.98");
    expect(peakAfterNorm).toBeCloseTo(0.98, 4);

    // Step 2: Compress
    const compressed = compressAudioBuffer(audioContext, normalized, 0.3, 4, 1.5);
